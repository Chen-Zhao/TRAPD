...
def extractcounts(pops, vcfline, max_ac, max_af, popmax_af,min_an):
	...
	if (ac_out>float(max_ac)) or (af_out>float(max_af)) or (an<float(min_an)) or (af_popmax_out>float(popmax_af)):
		...
		if options.database=="gnomad":
			if "ALL" in pops:
				if ";controls_ac=" in (";"+vcfline):
					ac_out=(";"+vcfline).split((";controls_ac="))[1].split(";")[0].split(",")[0]
				if ";controls_nhomalt=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";controls_nhomalt="))[1].split(";")[0].split(",")[0]
				elif ";controls_hom=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";controls_hom="))[1].split(";")[0].split(",")[0]
				else:
					hom_out=0

				if ";controls_an=" in (";"+vcfline):
					an_out=(";"+vcfline).split((";controls_an="))[1].split(";")[0].split(",")[0]
				...
				for p in range(0, len(pops), 1):
					...
					if (";controls_nhomalt_"+temp_pop+"=") in (";"+vcfline):
						hom_out=hom_out+num_convert((";"+vcfline).split((";controls_nhomalt_"+temp_pop+"="))[1].split(";")[0].split(",")[0], 0)
					elif (";controls_hom_"+temp_pop+"=") in (";"+vcfline):
						hom_out=hom_out+num_convert((";"+vcfline).split((";controls_hom_"+temp_pop+"="))[1].split(";")[0].split(",")[0],0)
					if (";controls_an_"+temp_pop+"=") in (";"+vcfline):
						an_out=hom_out+num_convert((";"+vcfline).split((";controls_an_"+temp_pop+"="))[1].split(";")[0].split(",")[0],0)

		elif options.database=="exac":
			if "ALL" in pops:
				if ";controls_ac_hom=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";controls_ac_hom="))[1].split(";")[0].split(",")[0]
				else:
					hom_out=0
				hom_out=num_convert(hom_out, 0)
			elif "ALL" not in pops:
...
def get_popmax(vcfline):
	...
	if options.database in ["gnomad", "generic"]:
		if ";controls_af_popmax=" in (";"+vcfline):
			af_popmax_out=num_convert((";"+vcfline).split((";controls_af_popmax="))[1].split(";")[0].split(",")[0],0)

	if options.database=="exac":
		if (";controls_ac_popmax=" in (";"+vcfline)) and (";controls_an_popmax" in (";"+vcfline)):
			ac_popmax=num_convert((";"+vcfline).split((";controls_ac_popmax="))[1].split(";")[0].split(",")[0],0)
			an_popmax=num_convert((";"+vcfline).split((";controls_an_popmax="))[1].split(";")[0].split(",")[0],100000000000)
