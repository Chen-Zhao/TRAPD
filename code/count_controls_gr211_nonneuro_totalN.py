...
if options.database=="generic":
	...
	for line_vcf1 in vcffile:
		line_vcf1=str(line_vcf1)
		if '#' in line_vcf1 and ("id=ac," in line_vcf1.lower()):
			ac_found=1
		elif '#' in line_vcf1 and ("id=an," in line_vcf1.lower()):
			an_found=1
		elif '#C' in line_vcf1:
			break
	...
	if (ac_out>float(max_ac)) or (af_out>float(max_af)) or (an<float(min_an)) or (af_popmax_out>float(popmax_af)):
		...
		if options.database=="gnomad":
			if "ALL" in pops:
				...
				if ";non_neuro_ac=" in (";"+vcfline):
					ac_out=(";"+vcfline).split((";non_neuro_ac="))[1].split(";")[0].split(",")[0]
				if ";non_neuro_nhomalt=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";non_neuro_nhomalt="))[1].split(";")[0].split(",")[0]
				elif ";non_neuro_hom=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";non_neuro_hom="))[1].split(";")[0].split(",")[0]
				else:
					hom_out=0

				if ";non_neuro_an=" in (";"+vcfline):
					an_out=(";"+vcfline).split((";non_neuro_an="))[1].split(";")[0].split(",")[0]
				...
				for p in range(0, len(pops), 1):
					...
					if (";non_neuro_ac_"+temp_pop+"=") in (";"+vcfline):
						ac_out=ac_out+num_convert((";"+vcfline).split((";non_neuro_ac_"+temp_pop+"="))[1].split(";")[0].split(",")[0],0)
					if (";non_neuro_nhomalt_"+temp_pop+"=") in (";"+vcfline):
						hom_out=hom_out+num_convert((";"+vcfline).split((";non_neuro_nhomalt_"+temp_pop+"="))[1].split(";")[0].split(",")[0], 0)
					elif (";non_neuro_hom_"+temp_pop+"=") in (";"+vcfline):
						hom_out=hom_out+num_convert((";"+vcfline).split((";non_neuro_hom_"+temp_pop+"="))[1].split(";")[0].split(",")[0],0)
					if (";non_neuro_an_"+temp_pop+"=") in (";"+vcfline):
						an_out=hom_out+num_convert((";"+vcfline).split((";non_neuro_an_"+temp_pop+"="))[1].split(";")[0].split(",")[0],0)

		elif options.database=="exac":
			if "ALL" in pops:
				if ";non_neuro_ac_hom=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";non_neuro_ac_hom="))[1].split(";")[0].split(",")[0]
				else:
					hom_out=0
				hom_out=num_convert(hom_out, 0)
			elif "ALL" not in pops:
...
def get_popmax(vcfline):
	...
	if options.database in ["gnomad", "generic"]:
		if ";non_neuro_af_popmax=" in (";"+vcfline):
			af_popmax_out=num_convert((";"+vcfline).split((";non_neuro_af_popmax="))[1].split(";")[0].split(",")[0],0)

	if options.database=="exac":
		if (";non_neuro_ac_popmax=" in (";"+vcfline)) and (";non_neuro_an_popmax" in (";"+vcfline)):
			ac_popmax=num_convert((";"+vcfline).split((";non_neuro_ac_popmax="))[1].split(";")[0].split(",")[0],0)
			an_popmax=num_convert((";"+vcfline).split((";non_neuro_an_popmax="))[1].split(";")[0].split(",")[0],100000000000)
...
def sumcount(genesnps, snptable):
	...
	for s in range(0, len(genesnps), 1):
		if genesnps[s] in snptable:
			...
			an_sum=an_sum+int(snptable[tempsnp][3])
			if int(snptable[tempsnp][3]) > 0:
				nsnps=nsnps+1
	if nsnps == 0:
		nsnps=1
	return [ac_sum, hom_sum, int(an_sum/nsnps)]
...
outfile.write("#GENE\tCONTROL_COUNT_HET\tCONTROL_COUNT_HOM\tCONTROL_TOTAL_AC\tCONTROL_TOTAL_ANmean\n")
...
for line_s1 in snpfile:
	...
	if line_s[0][0]!="#":
		...
		outfile.write(line_s[0]+"\t"+str(sumcounts[0]-2*sumcounts[1])+"\t"+str(sumcounts[1])+"\t"+str(sumcounts[0])+"\t"+str(sumcounts[2])+'\n')
outfile.close()
snpfile.close()
